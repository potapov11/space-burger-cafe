...
const IngredientBox = forwardRef(({ data, title, isModalOpen, openModal, closeModal }, ref) => {
	return (
		<div ref={ref} className={IngredientBoxCss.ingredientBox}>
			<h3 className="text text_type_main-medium mb-6">{title}</h3>
			<ul className={IngredientBoxCss.productsBox}>
				{data?.length > 0 && data.map((item) => <Ingredient key={item._id} item={item} {...item} isModalOpen={isModalOpen} openModal={openModal} closeModal={closeModal} />)}
			</ul>
		</div>
	);
});
