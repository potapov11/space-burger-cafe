...
const FeedListOrders = () => {
	...
	const arrayAllOrders = useSelector((store) => store.feedReducer.orders.orders);
	const isLoadingOrders = useSelector((store) => store.feedReducer.isLoading);
	const slicedArrayOrders: OrderFeed[] = arrayAllOrders?.slice(0, MAX_ORDERS_COUNT);

	console.log(arrayAllOrders, '..arrayAllOrders...');
